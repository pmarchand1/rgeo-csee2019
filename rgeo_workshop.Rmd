...
We can convert this data to an `sf` object with `st_as_sf`. The `coords` argument specify which columns hold the X and Y coordinates, while the `crs` argument defines the coordinate reference system (here, it is set to the same CRS as the MRC dataset).
...
* The `data` argument specifies the dataset to plot and the `aes` function associates variables in that dataset to graphical elements. These can be defined in the `ggplot` function (if they apply to all layers) on in specific `geom` layers. 
...
We can use `st_intersection` to extract portions of the `mrc_01_11` polygons within 10 km of a forest inventory plot, i.e. within the merged buffer created above, and use `st_difference` to extract areas of the MRC polygons outside that buffer.
...
All data in this workshop was made publicly available by Québec and Canada government agencies. The script `data_prep.R` shows the detailed steps to produce the workshop data files from the original sources.
